...
public class B2_JoSql_AL {
	...
	public static void main(String[] args) {
		...
		B2_JoSql_AL b2j = new B2_JoSql_AL();
		...
		for(int i = 0; i < Integer.parseInt(args[2]); i++){
			...
			if(args[0].contains("Phone_Numbers")){
				...
				b2j.parseDBank("SELECT * FROM java.util.ArrayList WHERE:_currobj LIKE '" + args[1] + "%' OR:_currobj LIKE '" + args[1] + "'");
			} else if(args[0].contains("Full_Names")){
				// Search for a matching string
				b2j.parseDBank("SELECT * FROM java.util.ArrayList WHERE:_currobj LIKE '" + args[1] + "%'");
			} else {
