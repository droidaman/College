...
$queryselected = "select A.CID,B.PID,C.username from posts_comments A,posts B, members C WHERE A.PID=B.PID AND A.USERID=C.USERID $addtosql $sort $sorthow limit $config[maximum_results]";
$query2 = "select A.*,B.PID,B.story,C.username from posts_comments A,posts B, members C WHERE A.PID=B.PID AND A.USERID=C.USERID $addtosql $sort $sorthow limit $pagingstart, $config[items_per_page]";
