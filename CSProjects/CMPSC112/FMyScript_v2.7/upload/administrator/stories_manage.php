...
$queryselected = "select A.PID,A.username from posts A WHERE A.PID>0 $addtosql $sort $sorthow limit $config[maximum_results]";
$query2 = "select A.*,A.username from posts A WHERE A.PID>0 $addtosql $sort $sorthow limit $pagingstart, $config[items_per_page]";
...
{
	$error = "Sorry, no stories were found.";
}
