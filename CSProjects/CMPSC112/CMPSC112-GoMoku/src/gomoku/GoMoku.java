...
public class GoMoku {
	...
	public void play() {
		...
		while (!GAME_OVER) {
			...
			//Subtract 1 from the user input here, convert input to properly access array, and check to see if the number is valid.
			int row = READER.nextInt()-1;
			while((row+1) < 1 || (row+1) > 19){
				System.out.println("Invalid input: There are only 19 rows, please choose again.\nRow: ");
				row = READER.nextInt()-1;
			}
			
			System.out.print("Column: ");
			//Subtract 1 from the user input here, convert input to properly access array.
			int column = READER.nextInt()-1;
			while((column+1) < 1 || (column+1) > 19){
				System.out.println("Invalid input: There are only 19 columns, please choose again.\nColumn: ");
